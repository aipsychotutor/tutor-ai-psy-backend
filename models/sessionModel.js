    ...
    let query = supabase
        ...
        .select(
            `
              ...
              patients (patient_name, profile_image)
            `
        )
        .order("start_time", { ascending: false })
        .eq("user_id", userId);

    if (queryFilters.patient_id) query = query.eq("patient_id", queryFilters.patient_id);
...
async function getSessionsByPatientIdWithEvaluation(patientId, userId) {
    ...
    if (error) throw new Error("Gagal mengambil sesi pasien dengan evaluasi: " + error.message);
    return sessions;
}
...
async function getSessionsWithEvaluationsByUserId(userId) {
    ...
    if (error) throw new Error("Gagal mengambil sesi untuk statistik user: " + error.message);
    return sessions;
}
...
async function updateSession(sessionId, userId, updateData) {
    ...
    if (error || !data) {
        throw new Error("Gagal update sesi: Session tidak ditemukan atau Anda tidak punya akses.");
    }
    return data;
}
